...
#define MAX_ECG_VAL 4095
#define NUMBER_OF_ECG_POINTS 1000


#if defined(__TI_COMPILER_VERSION__)
#pragma PERSISTENT(ecg_tab)
unsigned long ecg_tab[NUMBER_OF_ECG_POINTS] =
#elif defined(__IAR_SYSTEMS_ICC__)
__persistent unsigned long ecg_tab[NUMBER_OF_ECG_POINTS] =
#elif defined(__GNUC__)
unsigned long __attribute__((persistent)) ecg_tab[NUMBER_OF_ECG_POINTS] =
#else
...
#define NUMBER_OF_SINE_POINTS 1000

#if defined(__TI_COMPILER_VERSION__)
#pragma PERSISTENT(sine_tab)
unsigned short sine_tab[NUMBER_OF_SINE_POINTS] =
#elif defined(__IAR_SYSTEMS_ICC__)
__persistent unsigned long ecg_tab[NUMBER_OF_ECG_POINTS] =
#elif defined(__GNUC__)
unsigned long __attribute__((persistent)) sine_tab[NUMBER_OF_SINE_POINTS] =
#else
#error Compiler not supported!
#endif
...
// Stworzenie tablicy przeskalowanej funkcji sinus w pamiêci FRAM:
#if defined(__TI_COMPILER_VERSION__)
#pragma PERSISTENT(scaled_sine_tab)
unsigned long scaled_sine_tab[NUMBER_OF_SINE_POINTS] = {0};
#elif defined(__IAR_SYSTEMS_ICC__)
__persistent unsigned long scaled_sine_tab[NUMBER_OF_SINE_POINTS] = {0};
#elif defined(__GNUC__)
unsigned long __attribute__((persistent)) scaled_sine_tab[NUMBER_OF_SINE_POINTS] = {0};
#else
#error Compiler not supported!
#endif

void scale_sine_tab(int max_value)
{
    float scale = max_value / MAX_SINE_VAL;
    unsigned int i = NUMBER_OF_SINE_POINTS - 1;

    do // iterowanie od koñca, bo kompilator powiedzia³, ¿e tak jest szybciej xd
    {
       scaled_sine_tab[i] = (unsigned short) (scale * (float) sine_tab[i]);
       --i;
    } while (i != 0);
}
